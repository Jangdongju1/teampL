    ...
    public List<TeamMemberObj> getTeamMemberList(String email, Integer regNum) {
        ...
        try {
            ...
            list = queryFactory.select(Projections.constructor(TeamMemberObj.class,
                            teamMember.teamMemberPk.regNum,
                            user.email,
                            user.profileImg,
                            team.teamName
                    ))
                    .from(teamMember)
                    .innerJoin(user).on(teamMember.teamMemberPk.email.eq(user.email))
                    .innerJoin(team).on(teamMember.teamMemberPk.regNum.eq(team.regNum))
                    .fetchJoin()
